...
static int64_t s_inicio_pulsacion[BOTONES_N] = {0};
static int s_deslizamiento_paso = 0;
...
static void tarea_tactil(void *arg)
{
    ...
    while (1) {
        ...
        if (xQueueReceive(s_cola, &ev, espera) == pdTRUE) {
            ...
            eventos++;
            //// Opcional: Detectar pulsación corta/larga
            if (ev.activo) {
            // on_active: guardar cuándo comenzó la pulsación.
                ...
                const int64_t duracion_ms = duracion_us / 1000;

                const bool pulsacion_larga =
                duracion_ms >= PULSACION_LARGA_MS;

                ESP_LOGI(TAG,
                "T%d %s -> %s | duracion=%" PRId64 " ms",
                ev.canal,
                BOTONES[idx].nombre,
                ...
                if (ev.canal == 3 && s_deslizamiento_paso == 0) {
                    s_deslizamiento_paso = 1;
                    ESP_LOGI(TAG, "Deslizamiento: VOL_DOWN detectado");
                }
                else if (ev.canal == 2 && s_deslizamiento_paso == 1) {
                    s_deslizamiento_paso = 2;
                    ESP_LOGI(TAG, "Deslizamiento: PLAY detectado");
                }
                else if (ev.canal == 1 && s_deslizamiento_paso == 2) {
                    s_deslizamiento_paso = 0;
                    ESP_LOGI(TAG, "¡¡DESLIZAMIENTO VOL_DOWN -> PLAY -> VOL_UP DETECTADO!!");
                }
                else if (ev.canal != 3) {
                    // Si aparece otro botón en el medio, reiniciar.
                    s_deslizamiento_paso = 0;
                }
            }
        } else {
